...
import CoworkSpaceForm from './CoworkSpaceForm';

const CoworkSpaceModal = ({
  isOpen,
  onClose,
  initialData = null,
  onSubmit,
        ...
        <div className="px-6 py-4 border-b border-[#E5E5E5] flex items-center justify-between bg-white shrink-0">
          <div>
            <h2 className="text-lg font-bold text-[#000000] tracking-tight">
              {isEditMode ? 'Edit Cowork Space' : 'Add Cowork Space'}
            </h2>
            <p className="text-xs text-[#505050] mt-0.5">
              {isEditMode
                ? 'Update client details, seat allocations, and agreement.'
                : 'Register a new coworking space client and seat allocation.'}
            </p>
          </div>
          <button
            type="button"
...
export default CoworkSpaceModal;
