...
const DeleteManagedOfficeModal = ({
  isOpen,
  onClose,
  onConfirm,
  isLoading = false,
  ...
  if (!isOpen || !managedOffice) return null;

  const clientName = `${managedOffice.firstName || ''} ${managedOffice.lastName || ''}`.trim();
  const companyName = managedOffice.companyName || 'N/A';
  const officeNo = managedOffice.officeNo || 'N/A';
      ...
      >
        ...
        <div>
          <h3 className="text-xl font-bold text-black tracking-tight my-0">
            Delete Managed Office
          </h3>
          <p className="text-sm font-medium text-muted-text mt-2">
            Are you sure you want to delete this managed office? This action cannot be undone.
          </p>
        </div>

        <div className="bg-neutral-50 p-3.5 rounded-xl border border-neutral-200/80 text-left space-y-1.5 text-xs">
          <div className="flex justify-between">
            <span className="text-neutral-500 font-medium">Office No.:</span>
            <strong className="text-black font-bold">{officeNo}</strong>
          </div>
          <div className="flex justify-between">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => onConfirm(managedOffice.id || managedOffice._id)}
            disabled={isLoading}
...
export default DeleteManagedOfficeModal;
