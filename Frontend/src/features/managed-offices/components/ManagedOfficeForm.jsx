...
import { formatDateInput, validateAgreementFile } from '../utils/managedOffices.utils';
...
const managedOfficeSchema = z
  .object({
    ...
    officeNo: z.string().trim().min(1, 'Office number is required'),
    totalSeats: z.coerce
      .number({ invalid_type_error: 'Total seats must be a positive integer' })
      .int('Total seats must be a positive integer')
      .gt(0, 'Total seats must be at least 1'),
...
const ManagedOfficeForm = ({
  ...
  const {
    ...
    resolver: zodResolver(managedOfficeSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      phone: '',
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-black mb-1">
              Total Seats <span className="text-brand-red">*</span>
            </label>
            <input
              type="number"
              min="1"
          ...
          <div>
            <label className="block text-xs font-semibold text-black mb-1">
              Per Seat Cost (₹) <span className="text-brand-red">*</span>
            </label>
            <input
              type="number"
              step="any"
              {...register('perSeatCost')}
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-neutral-100">
        ...
        >
          {isLoading ? (
            ...
            <span>{isEditMode ? 'Save Changes' : 'Create Managed Office'}</span>
          )}
        </button>
      </div>
    </form>
...
export default ManagedOfficeForm;
