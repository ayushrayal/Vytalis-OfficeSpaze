...
import DedicatedSpaceForm from './DedicatedSpaceForm';

const DedicatedSpaceModal = ({
  isOpen,
  onClose,
  initialData = null,
  onSubmit,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200 font-urbanist">
      <div
        className="bg-white w-full max-w-2xl rounded-2xl border border-[#E5E5E5] shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-[#E5E5E5] flex items-center justify-between bg-white shrink-0">
          <div>
            <h2 className="text-lg font-bold text-[#000000] tracking-tight">
              {isEditMode ? 'Edit Dedicated Space' : 'Add Dedicated Space'}
            </h2>
            <p className="text-xs text-[#505050] mt-0.5">
              {isEditMode
                ? 'Update client details, seat allocations, and agreement.'
                : 'Register a new dedicated space client and seat allocation.'}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <DedicatedSpaceForm
            ...
            onCancel={onClose}
            isLoading={isLoading}
            onViewExistingAgreement={onViewExistingAgreement}
          />
        </div>
      </div>
    </div>
...
export default DedicatedSpaceModal;
