...
import { formatDateInput, validateAgreementFile, formatBusinessType } from '../utils/dedicatedSpace.utils';

// Email Regex
const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

// Zod Schema for Dedicated Space Form
const dedicatedSpaceSchema = z
  .object({
    firstName: z
      .string()
...
const DedicatedSpaceForm = ({
  ...
  const {
    ...
    resolver: zodResolver(dedicatedSpaceSchema),
    defaultValues: {
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 font-urbanist">
      ...
      <div className="mt-6 flex flex-col-reverse sm:flex-row items-stretch sm:items-center sm:justify-end gap-2.5 sm:gap-3 pt-4 border-t border-[#E5E5E5]">
        ...
        >
          ...
          <span>{isEditMode ? 'Save Changes' : 'Add Dedicated Space'}</span>
        </button>
      </div>
    </form>
  );
...
export default DedicatedSpaceForm;
