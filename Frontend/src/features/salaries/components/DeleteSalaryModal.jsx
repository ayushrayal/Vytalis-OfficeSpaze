...
const DeleteSalaryModal = ({
  ...
  const employeeName = salaryRecord.employeeName || 'N/A';
  const role = salaryRecord.role || 'N/A';
  const salary = salaryRecord.employeeSalary ? formatCurrencyINR(salaryRecord.employeeSalary) : 'N/A';
      ...
      >
        ...
        <div className="bg-neutral-50 p-3.5 rounded-xl border border-neutral-200/80 text-left space-y-1.5 text-xs">
          <div className="flex justify-between">
            ...
            <strong className="text-black font-bold">{employeeName}</strong>
          </div>
          <div className="flex justify-between">
            <span className="text-neutral-500 font-medium">Role:</span>
            <strong className="text-black font-semibold">{role}</strong>
          </div>
          <div className="flex justify-between">
            <span className="text-neutral-500 font-medium">Salary:</span>
            <strong className="text-black font-semibold">{salary}</strong>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            type="button"
            onClick={() => onConfirm(salaryRecord.id || salaryRecord._id)}
            disabled={isLoading}
