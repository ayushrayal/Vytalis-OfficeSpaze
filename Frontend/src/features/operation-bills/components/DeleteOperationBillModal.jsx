...
const DeleteOperationBillModal = ({
  isOpen,
  onClose,
  onConfirm,
  isLoading = false,
  ...
  if (!isOpen || !billRecord) return null;

  const expenseType = billRecord.expenseType || 'N/A';
  const dateStr = formatDateDisplay(billRecord.date);
  const uploadedBy = billRecord.uploadedBy || 'N/A';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200 font-urbanist">
      ...
      >
        <div className="flex items-center gap-3 text-[#ED1F23] mb-3">
          ...
          <h3 className="text-lg font-bold text-[#000000]">Delete Operation Bill</h3>
        </div>

        <p className="text-sm text-[#505050] mb-4">
          Are you sure you want to delete this operation bill? This action cannot be undone.
        </p>

        {/* Record Details Box (NO AMOUNT FIELD) */}
        <div className="p-3.5 bg-[#F5F0EB]/60 border border-[#E5E5E5] rounded-xl space-y-1.5 text-xs text-[#000000] mb-6">
          <div className="flex justify-between">
            <span className="text-[#505050]">Expense Type:</span>
            <span className="font-semibold text-right">{expenseType}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            <span className="text-[#505050]">Uploaded By:</span>
            <span className="font-medium text-right">{uploadedBy}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            type="button"
            onClick={() => onConfirm(billRecord.id || billRecord._id)}
            disabled={isLoading}
...
export default DeleteOperationBillModal;
